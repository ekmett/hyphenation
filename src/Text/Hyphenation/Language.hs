...
loadHyphenator language = do
  let langAffix = languageAffix language
  hyp <- Char8.unpack . decompress <$> (getDataFileName ("hyph-" ++ langAffix ++ ".hyp.txt.gz") >>= Lazy.readFile)
  pat <- Char8.unpack . decompress <$> (getDataFileName ("hyph-" ++ langAffix ++ ".pat.txt.gz") >>= Lazy.readFile)
  chr <- Char8.unpack . decompress <$> (getDataFileName ("hyph-" ++ langAffix ++ ".chr.txt.gz") >>= Lazy.readFile)
...
loadHyphenator language = return $ Hyphenator tryLookup (parsePatterns pat) (parseExceptions hyp) defaultLeftMin defaultRightMin
  where Just hyp = Char8.unpack . decompress . Lazy.fromStrict <$> lookup ("hyph-" ++ langAffix ++ ".hyp.txt.gz") hyphenatorFiles
        Just pat = Char8.unpack . decompress . Lazy.fromStrict <$> lookup ("hyph-" ++ langAffix ++ ".pat.txt.gz") hyphenatorFiles
        Just chr = Char8.unpack . decompress . Lazy.fromStrict <$> lookup ("hyph-" ++ langAffix ++ ".chr.txt.gz") hyphenatorFiles
        chrMap = IM.fromList (Prelude.lines chr >>= chrLine)
        tryLookup x = fromMaybe x $ IM.lookup (fromEnum x) chrMap
        (defaultLeftMin, defaultRightMin) = languageMins language
        langAffix = languageAffix language
#endif
