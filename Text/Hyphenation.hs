...
module Text.Hyphenation
  (
  -- * Basic English hyphenation
    hyphenate
  -- * Extended API
  , hyphenate'
  -- * default English hyphenation patterns
  , knuthLiangPatterns
  , extraPatterns
  , exceptions
  ) where
...
-- | build points for exceptions
score :: String -> [Int]
score (x:yzs@(y:zs))
  | y == '-'  = 1 : score zs
  | otherwise = 0 : score yzs
score [x] = [0]
score [] = []

hyphenate' nf patterns exceptions = check where
  tree = foldr insert (Trie [] IM.empty) patterns
  exceptionMap = HM.fromList $ map (\xs -> (filter (/= '-') xs, score xs)) exceptions
  check word
    | n <= 4 = [word]
    | Just parts <- HM.lookup ls exceptionMap = process [] word parts
    ...
    where
      ...
      walk allpts t i = step (V.toList (V.drop i work)) t
        where
          step (x:xs) (Trie ps m) = case IM.lookup (fromEnum x) m of
            Just t'@(Trie ps' _) -> do
               put i ps'
               step xs t'
            Nothing -> return ()
...
hyphenate = hyphenate' (\c -> let r = toLower c in if isLower r then r else '?')
                       (knuthLiangPatterns ++ extraPatterns ++ exceptionPatterns) exceptions

-- | Knuth and Liang's original hyphenation patterns from classic TeX.
-- In the public domain.
knuthLiangPatterns :: [String]
knuthLiangPatterns = [
...
exceptionPatterns = 
  [".a8s9s8o9c8i8a8t8e."
  ,".dec9lina9tion."
  ".a8s9s8o9c8i8a8t8e8s.",".d8e8c9l8i9n8a9t8i8o8n.",".o8b8l8i8g9a9t8o8r8y.",".p8h8i8l9a8n9t8h8r8o8p8i8c.",".p8r8e8s8e8n8t.",".p8r8e8s8e8n8t8s.",
  ".p8r8o8j8e8c8t.",".p8r8o8j8e8c8t8s.",".r8e8c8i9p8r8o8c8i8t8y.",".r8e9c8o8g9n8i9z8a8n8c8e.",".r8e8f9o8r9m8a9t8i8o8n.",".r8e8t9r8i9b8u9t8i8o8n.",".t8a9b8l8e."]
  ]
